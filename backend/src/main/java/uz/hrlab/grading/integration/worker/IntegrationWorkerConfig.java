 ...
 *
 * <p>Batch-4: {@code @Scheduled} re-dispatch ({@link WorkerReQueuer}) is enabled
 * by {@link WorkerSchedulingConfig}, which is excluded from the one-shot
 * {@code migrate} profile so the migrator JVM is not kept alive by a scheduler
 * thread.
 */
@Configuration
@EnableAsync
